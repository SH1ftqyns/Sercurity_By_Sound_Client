...
public class SinGenerator {
    ...
    public void gen(int genRate, int duration) {
        if (STATE_START == mState) {
            ...
            if (null != mListener) {
                mListener.onStartGen();  //���ü�������onStartGen
            }

            int n = mBits / 2;//����2����������
            int totalCount = (mDuration * mSampleRate) / 1000;  //�ܹ�����
            double per = (mGenRate / (double) mSampleRate) * 2 * Math.PI;  //2*pi*����Ƶ��/Ԥ��Ƶ��
            double d = 0;

            LogHelper.d(TAG, "genRate:" + genRate);
            if (null != mCallback) {
                mFilledSize = 0;
                BufferData buffer = mCallback.getGenBuffer(); //���ûص��������ɳ�����
                if (null != buffer) {
